...
import { Sparkles, Loader2, Copy, FileText, CheckCheck, FileDown, Save, CheckCircle2 } from 'lucide-react';
...
export default function AIDocModal({ docType, open, onClose, onSaved }) {
  ...
  // Load existing saved document when modal opens
  useEffect(() => {
    ...
    base44.entities.PlatformDocument.filter({ doc_type: docType })
      .then(docs => {
  ...
  const handleSave = async () => {
    ...
    try {
      if (existingDoc) {
        ...
        setExistingDoc(created);
      }
      setSaved(true);
      if (onSaved) onSaved(docType);
    } finally {
      setSaving(false);
    }
  };
        ...
        <DialogHeader>
          ...
          {existingDoc && !content && (
            <p className="text-xs text-emerald-400 flex items-center gap-1">
              <CheckCircle2 className="w-3.5 h-3.5" /> Previously saved version loaded (v{existingDoc.version || 1})
            </p>
          )}
        </DialogHeader>

        <div className="flex-1 overflow-y-auto space-y-4 pr-1">
          {/* Input Fields — shown when no content yet */}
          {!content && !loading && (
            ...
            <div className="relative">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm font-medium text-emerald-400">
                  {saved ? '✓ Saved to platform' : 'Document generated'}
                </span>
